...
final public class FastPanelList<T extends JPanel> {
    public enum FPLOrientation {
        HORIZONTAL(Adjustable.HORIZONTAL), VERTICAL(Adjustable.VERTICAL);
     ...
     */
    public FastPanelList(final FPLOrientation orientation, final Function<Integer, T> panelSupplier, final double fractionOfExtentToScrollPerArrowClick, final double fractionOfExtentToScrollPerTrackClick, final double fractionOfExtentToScrollPerMouseWheelStep, final boolean hideScrollbarWhenUnnecessary, final int panelSize, final int panelCount) {
        if(orientation == null) {
    ...
    public void mouseWheelEvent(final MouseWheelEvent e) {
        ...
        final int increment = (int) Math.max(1, Math.min(extent, extent * fractionOfExtentToScrollPerMouseWheelStep));
    ...
    private int determineScrollBarDefaultWidth() { // Called only ONE time.
        final JScrollPane dummyForDefaultSize = new JScrollPane(ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_ALWAYS);
    ...
    public T getItemUnderMouse(final int xInComponent, final int yInComponent) {
     ...
     */
    public T getItem(final Integer index, final boolean callSupplierIfNotCached) {
    ...
    public void update() {
        ...
        final int unitIncrement = (int) Math.max(1, Math.min(extent, extent * fractionOfExtentToScrollPerArrowClick));
        final int blockIncrement = (int) Math.max(1, Math.min(extent, extent * fractionOfExtentToScrollPerTrackClick));
    ...
    public Dimension getPanelSize() {
        ...
        if(orientation == FPLOrientation.HORIZONTAL) {
            return new Dimension(panelSize, lastKnownContainerSize.height - (scrollBar.isVisible() ? scrollBarWidth : 0));
        } else {
            return new Dimension(lastKnownContainerSize.width - (scrollBar.isVisible() ? scrollBarWidth : 0), panelSize);
        }
    }
